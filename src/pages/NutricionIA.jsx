...
export default function NutricionIA() {
  ...
  async function saveToLog() {
    const { error: dbError } = await supabase.from('nutrition_logs').insert({
      ...
      notes: `Analizado con Gemini AI — original: "${input}"`,
    })
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p>Describí lo que comiste y Gemini AI calcula los macros automáticamente — gratis</p>
      </div>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        <textarea
          ...
          placeholder={`Describí lo que comiste...\n\nEjemplos:\n• "Almorcé una milanesa de ternera mediana con ensalada mixta"\n• "Desayuné 2 huevos revueltos, 2 tostadas con palta y café con leche"\n• "Comí un plato de arroz integral con pollo a la plancha"`}
          style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 12 }}>
          <span style={{ fontSize: 'var(--text-xs)', color: 'var(--gray-400)' }}>Ctrl+Enter para analizar</span>
          <button
            onClick={analyzeFood}
          ...
          >
            {loading ? (
              <><div className="spinner" style={{ width: 16, height: 16, borderWidth: 2 }} />Analizando...</>
            ) : (
              <>✨ Analizar con Gemini</>
            )}
          </button>
      ...
      {saved && <div className="form-success" style={{ marginBottom: 16 }}>✅ Guardado en tu registro nutricional</div>}
      ...
      {result && (
        <div className="card" style={{ marginBottom: 20 }}>
          ...
          <div className="form-group" style={{ marginBottom: 20 }}>
            <label>Descripción normalizada</label>
            <input type="text"
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 12, marginBottom: 20 }}>
            {macroFields.map(f => (
              <div key={f.key} style={{
                border: `2px solid ${f.color}20`, borderRadius: 'var(--radius)',
                padding: '14px 12px', background: `${f.color}08`, textAlign: 'center',
              }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    borderBottom: `2px solid ${f.color}`, background: 'transparent',
                    fontSize: '1.1rem', fontWeight: 700, color: f.color, padding: '2px 0',
                  }}
                />
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 16 }}>
            <button onClick={saveToLog} className="btn-primary">💾 Guardar en mi registro</button>
            <button onClick={() => { setResult(null); setInput('') }} className="btn-secondary">Cancelar</button>
          </div>
        </div>
      )}
        ...
        <div className="card" style={{ marginTop: 16 }}>
          ...
          {historyLoading ? (
            ...
            <table className="data-table">
              <thead>
                <tr><th>Fecha</th><th>Comida</th><th>Descripción</th><th>Cal</th><th>Prot</th><th>Carbos</th><th>Grasas</th></tr>
              </thead>
              <tbody>
                {history.map(log => {
...
function MacroBar({ proteinas, carbos, grasas }) {
  ...
  return (
    <div>
      <div style={{ fontSize: 'var(--text-xs)', color: 'var(--gray-500)', marginBottom: 6 }}>Distribución de macros</div>
      <div style={{ display: 'flex', height: 20, borderRadius: 999, overflow: 'hidden', gap: 2 }}>
        <div style={{ width: `${pProt}%`, background: '#2ecc71', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: 'white', fontWeight: 700 }}>{pProt > 8 ? `${pProt}%` : ''}</div>
        <div style={{ width: `${pCarb}%`, background: '#f39c12', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: 'white', fontWeight: 700 }}>{pCarb > 8 ? `${pCarb}%` : ''}</div>
        <div style={{ width: `${pGras}%`, background: '#9b59b6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: 'white', fontWeight: 700 }}>{pGras > 8 ? `${pGras}%` : ''}</div>
      </div>
