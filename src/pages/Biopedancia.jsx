...
export default function Biopedancia() {
  ...
  const [form, setForm] = useState({
    ...
    body_fat_pct: '', muscle_mass_kg: '', bone_mass_kg: '',
    water_pct: '', visceral_fat: '', metabolic_age: '',
    bmi: '', basal_metabolic_rate: '', notes: ''
  })
  ...
  async function saveRecord(e) {
    e.preventDefault()
    const payload = { user_id: user.id, date: form.date, notes: form.notes }
    const numFields = ['body_fat_pct', 'muscle_mass_kg', 'bone_mass_kg', 'water_pct', 'visceral_fat', 'metabolic_age', 'bmi', 'basal_metabolic_rate']
    ...
    showMsg('success', '✅ Biopedancia registrada')
    setForm(f => ({ ...f, body_fat_pct: '', muscle_mass_kg: '', bone_mass_kg: '', water_pct: '', visceral_fat: '', metabolic_age: '', bmi: '', basal_metabolic_rate: '', notes: '' }))
    loadRecords()
  }
  ...
  return (
    <div>
      ...
      {tab === 'registrar' && (
        <div className="card">
          <div className="card-title">➕ Nueva biopedancia</div>
          <p style={{ fontSize: 'var(--text-sm)', color: 'var(--gray-500)', marginBottom: 20 }}>
            Completá los datos que te da tu báscula de biopedancia. Solo los campos que disponés.
          </p>
          <form onSubmit={saveRecord}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 16, marginBottom: 16 }}>
              {[
                { key: 'body_fat_pct',       label: '% Grasa corporal',       unit: '%',  step: '0.1', placeholder: 'ej: 28.5' },
                { key: 'muscle_mass_kg',      label: 'Masa muscular',          unit: 'kg', step: '0.1', placeholder: 'ej: 55.2' },
                { key: 'bone_mass_kg',        label: 'Masa ósea',             unit: 'kg', step: '0.01', placeholder: 'ej: 2.8' },
                { key: 'water_pct',           label: '% Agua corporal',        unit: '%',  step: '0.1', placeholder: 'ej: 50.3' },
                { key: 'visceral_fat',        label: 'Grasa visceral (nivel)', unit: '',   step: '1',   placeholder: 'ej: 8' },
                { key: 'metabolic_age',       label: 'Edad metabólica',        unit: 'años', step: '1', placeholder: 'ej: 35' },
                { key: 'bmi',                 label: 'IMC',                    unit: '',   step: '0.1', placeholder: 'ej: 27.5' },
                { key: 'basal_metabolic_rate',label: 'Metabolismo basal',      unit: 'kcal', step: '1', placeholder: 'ej: 1650' },
              ].map(field => (
                <div className="form-group" key={field.key}>
                  <label>{field.label} {field.unit && <span style={{ color: 'var(--gray-400)', fontSize: 'var(--text-xs)' }}>({field.unit})</span>}</label>
                  <input type="number" step={field.step} min="0" placeholder={field.placeholder}
            ...
            <div className="form-group" style={{ marginBottom: 16 }}>
              <label>Notas</label>
              <textarea placeholder="Observaciones, condiciones del estudio, etc."
                value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} />
            </div>

            <button type="submit" className="btn-primary">Guardar biopedancia</button>
          </form>
        </div>
      )}
